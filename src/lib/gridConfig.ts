...
// GRID_PREVIEW_FRAME_CHROME below for why the width fed to the grid itself
// ends up smaller than these numbers.
...
// .grid-preview-frame (grid.scss) is deliberately content-box, so its
// padding/border sit *outside* the width passed to the grid rather than
// eating into it — otherwise react-grid-layout's column math would assume
// more space than the frame actually has room for, and widgets would poke
// out past the frame's edge. The tradeoff is that the frame's own padding +
// border then add to GRID_PREVIEW_WIDTHS rather than being included in it,
// so the on-screen frame ends up wider than the width it's meant to
// simulate — on mobile specifically, wide enough to overflow the real
// viewport, since a phone can enter edit mode too (see page.tsx's
// allowedBreakpoints). Subtract this from GRID_PREVIEW_WIDTHS before handing
// it to the grid so the frame's total footprint (padding/border included)
// matches the simulated width instead of exceeding it. Kept in sync by hand
// with grid.scss's `padding: 1rem` (16px) + `border: 1px` on
// .grid-preview-frame, same pattern as orbit-animation.scss's path constants.
