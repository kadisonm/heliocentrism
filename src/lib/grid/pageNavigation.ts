...
// only while a widget drag/resize is in flight (see Grid.tsx's
// isDragActive), and hidden once the breakpoint hits the page cap (which is
// what actually disables page creation, not a check elsewhere).
export function buildVirtualPages(pages: DashboardPage[], allowBlankSlot: boolean): VirtualPage[] {
  const virtual: VirtualPage[] = pages.map((page) => ({ kind: 'real', page }));
  if (allowBlankSlot && pages.length < MAX_PAGES_PER_BREAKPOINT) virtual.push({ kind: 'blank' });
  return virtual;
}

// index === pageCount (the blank slot) is only ever valid while a drag is
// making it reachable. This one rule is the entire implementation of "the
// slide back off the blank page snaps to the last real page" once the drag
// that revealed it ends.
export function clampPageIndex(index: number, pageCount: number, allowBlankSlot: boolean): number {
  const maxIndex = allowBlankSlot ? pageCount : Math.max(0, pageCount - 1);
