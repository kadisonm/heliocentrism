...
export function usePageNavigation(
  ...
  const virtualPages = useMemo(() => buildVirtualPages(pages, allowBlankSlot), [pages, allowBlankSlot]);
  const activeIndex = clampPageIndex(rawIndex, pages.length, allowBlankSlot);
  ...
  const goToIndex = useCallback(
    (index: number) => {
      const clamped = clampPageIndex(index, pages.length, allowBlankSlot);
      if (clamped !== activeIndex) onChange(clamped);
    },
    [pages.length, allowBlankSlot, activeIndex, onChange]
  );
